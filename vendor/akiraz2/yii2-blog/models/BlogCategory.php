...
class BlogCategory extends \yii\db\ActiveRecord
{
    ...
    public static function getOneIsNavLabel($isNav = null)
    {
        ...
        return isset($arrayIsNav[$isNav])?$arrayIsNav[$isNav]:'Không tồn tại';
    }
    ...
    static public function getCategory($parentId = 0, $array = array())
    {
        ...
        foreach ((array)$array as $v) {
            if ($v['parent_id'] == $parentId) {
                $newArray[$v['id']] = array(
                    'text' => $v['title'] . ' Menu[' . ($v['is_nav'] ? Module::t('blog', 'CONSTANT_YES') : Module::t('blog', 'CONSTANT_NO')) . '] Sắp xếp[' . $v['sort_order'] .
                        '] Kiểu[' . ($v['page_type'] == 'list' ? Module::t('blog', 'PAGE_TYPE_LIST') : Module::t('blog', 'PAGE_TYPE_PAGE')) . '] Trạng thái[' .
                        F::getStatus2($v['status']) . '] [<a href="' . Yii::app()->createUrl('/category/update', array('id' => $v['id'])) . '">Sủa đổi</a>][<a href="'
                        . Yii::app()->createUrl('/category/create', array('id' => $v['id'])) . '">Thêm menu con</a>]&nbsp;&nbsp[<a href="' .
                        Yii::app()->createUrl('/category/delete', array('id' => $v['id'])) . '">Xóa bỏ</a>]',
                    //'children'=>array(),
                );
    ...
    public function attributeLabels()
    {
        return [
            'id' => Module::t('blog', 'ID'),
            'parent_id' => Module::t('blog', 'Danh mục'),
