var imageManagerModule = {
  ...
  filterImageResult: function (searchTerm) {
    //set new url
    var newUrl = window.queryStringParameter.set(
      window.location.href,
      "ImageManagerSearch[globalSearch]",
      searchTerm
    );
    //set pjax
    $.pjax({
  ...
  selectImage: function (id) {
    ...
    $(
      "#module-imagemanager .item-overview .item[data-key='" + id + "']"
    ).addClass("selected");
    //get details
  ...
  pickImage: function () {
    ...
    switch (imageManagerModule.selectType) {
      ...
      case "input":
        ...
        //set input data
        $("#" + sFieldId, window.parent.document).val(
          imageManagerModule.selectedImage.id
        );
        $("#" + sFieldNameId, window.parent.document).val(
          imageManagerModule.selectedImage.file_name
        );
        $("#" + sFieldImageId, window.parent.document)
          .attr("src", imageManagerModule.selectedImage.image)
          .parent()
          .removeClass("hide");
        //trigger change
        ...
        //show delete button
        $(
          ".delete-selected-image[data-input-id='" + sFieldId + "']",
          window.parent.document
        ).removeClass("hide");
        //close the modal
        window.parent.imageManagerInput.closeModal();
        console.log(imageManagerModule);
        imageManagerModule.saveSelectedImage();
        break;
      //CKEditor selector
      case "ckeditor":
      //TinyMCE Selector
          ...
          $.ajax({
            ...
            success: function (responseData, textStatus, jqXHR) {
              ...
              if (imageManagerModule.selectType == "ckeditor") {
                var sField = window.queryStringParameter.get(
                  window.location.href,
                  "CKEditorFuncNum"
                );
  ...
  //save the selected image to user
  saveSelectedImage: function () {
    //check if isset image
    //console.log(imageManagerModule.baseUrl + "/save-user-image");
    if (imageManagerModule.selectedImage !== null) {
      //call action by ajax
      $.ajax({
        url: imageManagerModule.baseUrlUser,
        type: "POST",
        data: {
          ImageManager_id: imageManagerModule.selectedImage.id,
          file_name: imageManagerModule.selectedImage.file_name,
          _csrf: $("meta[name=csrf-token]").prop("content"),
        },
        dataType: "json",
        success: function (responseData, textStatus, jqXHR) {
          //check if delete is true
          if (responseData.save === true) {
            console.log("Save image user is successfuly");
          } else {
            alert("Error: item is not saved");
          }
        },
        error: function (jqXHR, textStatus, errorThrown) {
          alert("Error: can't saved item");
        },
      });
    } else {
      alert("Error: image can't save, no image isset set");
    }
  },
        ...
        $.ajax({
          ...
          success: function (responseData, textStatus, jqXHR) {
            ...
            if (responseData.delete === true) {
              //delete item element
              $(
                "#module-imagemanager .item-overview .item[data-key='" +
                  imageManagerModule.selectedImage.id +
                  "']"
              ).remove();
  ...
  getDetails: function (id, pickAfterGetDetails) {
    ...
    pickAfterGetDetails =
      pickAfterGetDetails !== undefined ? pickAfterGetDetails : false;
    //call action by ajax
    $.ajax({
      ...
      success: function (responseData, textStatus, jqXHR) {
        ...
        if (pickAfterGetDetails) {
          ...
          //set text elements
          $("#module-imagemanager .image-info .fileName")
            .text(responseData.file_name)
            .attr("title", responseData.file_name);
          $("#module-imagemanager .image-info .created").text(
            responseData.created
          );
          $("#module-imagemanager .image-info .fileSize").text(
            responseData.fileSize
          );
          $(
            "#module-imagemanager .image-info .dimensions .dimension-width"
          ).text(responseData.dimensionWidth);
          $(
            "#module-imagemanager .image-info .dimensions .dimension-height"
          ).text(responseData.dimensionHeight);
          $("#module-imagemanager .image-info .thumbnail").html(
            "<img src='" +
              responseData.image +
              "' alt='" +
    ...
    openCropper: function () {
      ...
      if (imageManagerModule.selectedImage !== null) {
        ...
        $.ajax({
          ...
          success: function (responseData, textStatus, jqXHR) {
            //hide cropper
            $("#module-imagemanager > .row .col-image-cropper").css(
              "visibility",
              "hidden"
            );
            ...
            $(
              ...
              .cropper(
                "setAspectRatio",
                parseFloat(imageManagerModule.cropRatio)
              )
...
$(document).ready(function () {
  ...
  $(document).on(
    "click",
    "#module-imagemanager .item-overview .item",
    function () {
      //get id
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-info .pick-image-item",
    function () {
      imageManagerModule.pickImage();
      return false;
    }
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-info .delete-image-item",
    function () {
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-info .crop-image-item",
    function () {
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-cropper .apply-crop",
    function () {
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-cropper .apply-crop-select",
    function () {
  ...
  $(document).on(
    "click",
    "#module-imagemanager .image-cropper .cancel-crop",
    function () {
      imageManagerModule.editor.close();
      return false;
    }
