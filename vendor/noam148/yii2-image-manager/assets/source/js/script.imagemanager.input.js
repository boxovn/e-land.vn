var imageManagerInput = {
  ...
  initModal: function () {
    ...
    if ($("#modal-imagemanager").length === 0) {
      ...
      var sModalHtml =
        '<div tabindex="-1" role="dialog" class="fade modal" id="modal-imagemanager">';
      ...
      sModalHtml += "<h4>Quản lý hình ảnh</h4>";
      sModalHtml += "</div>";
  ...
  openModal: function (inputId, aspectRatio, cropViewMode) {
    ...
    //create iframe url
    var queryStringStartCharacter =
      imageManagerInput.baseUrl.indexOf("?") == -1 ? "?" : "&";
    var imageManagerUrl =
      imageManagerInput.baseUrl +
      queryStringStartCharacter +
      "view-mode=iframe&input-id=" +
      inputId +
      "&aspect-ratio=" +
      aspectRatio +
      "&crop-view-mode=" +
    ...
    $("#modal-imagemanager .modal-dialog .modal-header h4").text(
      imageManagerInput.message.imageManager
    );
    //open modal
  ...
  deletePickedImage: function (inputId) {
    ...
    var bShowConfirm = JSON.parse(
      $(".delete-selected-image[data-input-id='" + inputId + "']").data(
        "show-delete-confirm"
      )
    );
    ...
    $("#" + sImagePreviewId)
      ...
      .addClass("hide");
    //delete hide class
    $(".delete-selected-image[data-input-id='" + inputId + "']").addClass(
      "hide"
    );
  },
};
