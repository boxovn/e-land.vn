...
class ManagerController extends Controller {
	...
	public function actionIndex() {
		...
		$sBaseUrlUser = Url::to(['/user/save-user-image']);
		//set base url
		$this->view->registerJs("imageManagerModule.baseUrl = '" . $sBaseUrl . "';", 3);
		$this->view->registerJs("imageManagerModule.baseUrlUser = '" . $sBaseUrlUser . "';", 3);
		...
		if (isset($_FILES['imagemanagerFiles']['tmp_name'])) {
			...
			foreach ($_FILES['imagemanagerFiles']['tmp_name'] AS $key => $sTempFile) {
				...
				if ($iErrorCode == 0) { 
					...
					$model->file_hash = Yii::$app->getSecurity()->generateRandomString(32);
					$model->user_id = Yii::$app->user->identity->id;
					$model->file_name = strtotime('now') . "_" . $model->file_hash . "." . $sFileExtension; //str_replace("_", "-", $sFileName);
					//if file is saved add record
					if ($model->save()) {
						//move file to dir
						//$sSaveFileName = $model->id . "_" . $model->file_hash . "." . $sFileExtension;
						//move_uploaded_file($sTempFile, $sMediaPath."/".$sFileName);
						//save with Imagine class
						Image::getImagine()->open($sTempFile)->save($sMediaPath . "/" . $model->file_name);
						$bSuccess = true;
					}
				}
			}
	...
	public function actionCrop() {
		...
		if ($modelOriginal->imagePathPrivate !== null && $aCropData !== null) {
			...
			//collect variables
			$sFileNameReplace = preg_replace("/_crop_\d+x\d+/", "", $modelOriginal->file_name);
			...
			//create a file record
			$model = new ImageManager();
			$model->file_name = $sDisplayFileName;
			$model->file_hash = Yii::$app->getSecurity()->generateRandomString(32);
			//if file is saved add record
			if ($model->save()) {
...
//                       
	...
	protected function findModel($id) {
		...
		if (($model = ImageManager::findOne(['id' =>$id,'user_id' => Yii::$app->user->identity->id])) !== null) {
            ...
            if ($module->setBlameableBehavior) {
                ...
                if (Yii::$app->user->id != $model->createdBy) {
                    throw new NotFoundHttpException(Yii::t('imagemanager', 'The requested image does not exist.' .Yii::$app->user->id));
                }
            }
