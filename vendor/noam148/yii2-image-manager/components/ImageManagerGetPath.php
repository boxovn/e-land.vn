...
class ImageManagerGetPath extends Component { 
	...
	public function getImagePath($ImageManager_id, $width = 400, $height = 400, $thumbnailMode = "outbound") {
		//default return
		$return =  Yii::getAlias('@web') . '/images/no-image200x200.png';
		...
		if ($mImageManager !== null) {
			...
			$sFileExtension = pathinfo($mImageManager->file_name, PATHINFO_EXTENSION);
			//get image file path
			//$sImageFilePath = $sMediaPath . '/' . $mImageManager->id . '_' . $mImageManager->file_hash . '.' . $sFileExtension;
			$sImageFilePath = $sMediaPath . '/' . $mImageManager->file_name;
			//check file exists
			//check file exists
			if (file_exists($sImageFilePath)) {
				//$return = Url::to('@web/channels/avatar/'   . $mImageManager->id . '_' . $mImageManager->file_hash . '.' . $sFileExtension);
				$return = Url::to('@web/channels/avatar/'   . $mImageManager->file_name);
				//Yii::$app->imageresize->getUrl($sImageFilePath, $width, $height, $thumbnailMode, null, $mImageManager->file_name);
			} else {
				return $return; //$return = 'http://www.eland.local/images/no-image200x200.png'; //null; //isset(\Yii::$app->controller->module->assetPublishedUrl) ? \Yii::$app->controller->module->assetPublishedUrl. "/img/img_no-image.png" : null;
			}
		}
		return $return;
	}
