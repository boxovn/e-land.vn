...
class ImageManagerInputWidget extends InputWidget {
    ...
    public function run() {
        ...
        if ($this->hasModel()) {
            ...
            //get file_name from selected file
            $ImageManager_id = $this->model->{$sFieldAttributeName};
            $ImageManager_file_name = null;
            $mImageManager = ImageManager::findOne($ImageManager_id);
            if ($mImageManager !== null) {
                $ImageManager_file_name = $mImageManager->file_name;
            }
            //create field
            $field .= Html::hiddenInput($this->attribute, $ImageManager_file_name, ['class' => 'form-control', 'id' => $sFieldNameId, 'readonly' => true]);
    ...
    //    $field .= "<a href='#' class='input-group-addon btn btn-primary delete-selected-image " . $sHideClass . "' data-input-id='" . $sFieldId . "' data-show-delete-confirm='" . ($this->showDeletePickedImageConfirm ? "true" : "false") . "'><i class='glyphicon glyphicon-remove' aria-hidden='true'></i></a>";
        $field .= "<a style='width:120px; height:120px; float:left; border: 1px solid #ddd' href='#' class='open-modal-imagemanager' data-aspect-ratio='" . $this->aspectRatio . "' data-crop-view-mode='" . $this->cropViewMode . "' data-input-id='" . $sFieldId . "'>";
      //  $field .= "<i class='glyphicon glyphicon-folder-open' aria-hidden='true'></i>";
        

        //show preview if is true
        if ($this->showPreview == true) {
            $sHideClass = ($mImageManager == null) ? "" : "";
            $sImageSource = isset($mImageManager->id) ? \Yii::$app->imagemanager->getImagePath($mImageManager->id, 500, 500, 'inset') :  Yii::getAlias('@web') . '/images/no-image200x200.png';

            $field .= '<div class="image-wrapper ' . $sHideClass . '">' . '<img id="' . $sFieldId . '_image" alt="Thumbnail" class="img-responsive img-preview" src="' . $sImageSource . '">';
            $field .= "<label for='wid-avatar' class='wid-avatar-overlay'><i class='fa fa-camera' aria-hidden='true'></i> Avatar</label>";
            $field .= '</div>';
        }
        $field .= "</a></div>";
    ...
    public function registerClientScript() {
        ...
        $view->registerJs("imageManagerInput.message = " . Json::encode([
                    'imageManager' => Yii::t('imagemanager','Quản lý hình ảnh'),
