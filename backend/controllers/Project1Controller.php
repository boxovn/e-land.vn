...
class Project1Controller extends AppController
{
    /**
    ...
    {
    	$objectData = Yii::$app->request->get('ProjectSearch');
    	...
    	$uriParam = '&ProjectSearch[province_id]='.$provinceID;
    	$uriParam .= '&ProjectSearch[district_id]='.$districtID;
    	$uriParam .= '&ProjectSearch[ward_id]='.$wardID;
    	$uriParam .= '&ProjectSearch[name]='.$name;
    	$uriParam .= '&ProjectSearch[email]='.$email;
    ...
    public function actionEdit($id = 0) 
    {
        ...
        if (Yii::$app->request->isPost) {
    		...
    		if ($model->validate()) {
    			...
    			Yii::$app->getResponse()->redirect(['project/index', 'page' => isset($urlDataParams['pageBack'])?$urlDataParams['pageBack']:1]);
    			Yii::$app->end();
    		} else {
    			// Delete image upload on server and in DB
    			$sID = Yii::$app->request->post('upload_image_id');
    ...
    public function actionView($id = 0) 
    {
    	...
    	$apartmentCatalog = CategoryType::findOne(['id' => $infos->category_type_id]);
