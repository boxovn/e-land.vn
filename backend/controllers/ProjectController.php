	...
	public function actionProjectSection($id) 
    {
		...
		if($id &&  $model ){  
				...
				if(!$sections){
							...
							$sections= [
										new ProjectSection(['name' => 'Giới tiệu','sort' => 0]),
										new ProjectSection(['name' => 'Tổng quan','sort' => 0]),
										new ProjectSection(['name' => 'Vị Trí','sort' => 0]),
										new ProjectSection(['name' => 'Tiện ích','sort' => 0]),
										new ProjectSection(['name' => 'Thiết kế','sort' => 0])
										];
...
*/
    ...
    public function actionIndex($id=0)
    {
		...
		if (Yii::$app->request->isPost) { 
				...
				$model->ogirinal_price_from = $model->ogirinal_price_from?str_replace(',','', $model->ogirinal_price_from):0;
				$model->ogirinal_price_to = $model->ogirinal_price_to?str_replace(',','', $model->ogirinal_price_to):0;
				$model->market_price_from = $model->market_price_from?str_replace(',','', $model->market_price_from):0;
				$model->market_price_to = $model->market_price_to?str_replace(',','', $model->market_price_to):0;
				$model->hire_price_from = $model->hire_price_from?str_replace(',','', $model->hire_price_from):0;
				$model->hire_price_to = $model->hire_price_to?str_replace(',','', $model->hire_price_to):0;
				
				//$model->area = str_replace(',', '', $model->area);
				//	$model->description = strip_tags($model->content);
		...
		$projectCategories = ArrayHelper::map(ProjectCategory::find()->orderBy(['title' => 'ASC'])->all(),'id','title');
		$serviceCategories = ServiceCategory::find()->orderBy(['name' => 'ASC'])->asArray()->all();
	...
	*/
	...
	public function actionView($id = 0)
	{
				...
				$apartmentCatalog = ProjectCategory::findOne(['id' => $infos->category_id]);
