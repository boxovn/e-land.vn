...
{ 
 ...
 public function actionDistricts() 
     ...
     public function actionConvert($id)
    {
        ...
        $provinces = ArrayHelper::map(Province::find()->orderBy('type asc, name desc')->all(), 'province_id', function ($province) {
            ...
            $districts = ArrayHelper::map(District::find()->andWhere(['province_id' => $house->province_id])->orderBy('type desc, name asc')->all(), 'district_id', function ($district) {
             ...
             $articleTypes = ArrayHelper::map(CategoryType::find()->orderBy('title asc')->all(), 'id', function ($district) {
    ...
    {	
        ...
        $wards = ArrayHelper::map(Ward::find()->orderBy('type desc, name asc')->all(), 'ward_id', function ($ward_id) {
    ...
    public function actionUpdate($id)
    {
        ...
        $houseSegments = ArrayHelper::map(HouseSegment::find()->orderBy('id asc')->all(), 'id', function ($house_segment) {
            ...
            return $this->render('update', [
                ...
                'houseSegments' => $houseSegments,
                //'employees' => $employees,
				'dropzoneImage' => $this->dropzoneArticleImages($id),
			]);
    }
    private function dropzoneArticleImages($id){
       $articleImages = ArticleImage::find()
        				->where(['house_id' => $id])
                        ->all();
                        
		$dropzoneFile = array();
        if ($articleImages) {
        	foreach ($articleImages as  $item) {
                $dropzoneFile[] =[
					'name' => $item->image,
					'file_name' =>  $item->image,
                	'id' => $item->id,
        			'url_file_name' => Yii::$app->params['url-page'].'/channels/article/210x118/'. $item->image,
					'type' =>'image/*',
					'size' => '100',//$item->size,
				];
        	}
