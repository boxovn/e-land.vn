...
$(function () {
  ...
  // event listeners
  var myDropzoneLogoUpload = new Dropzone("#dropzone-logo-upload", {
  ...
  myDropzoneLogoUpload.on("thumbnail", function (file, thumb) {
    file.thumbnail = thumb;
    myDropzoneLogoUpload.processQueue();
  });
  myDropzoneLogoUpload.on("sending", function (file, xhr, formData) {
    formData.append("thumbnail", file.thumbnail);
  });
  myDropzoneLogoUpload.on("addedfile", function (file, response) {
    switch (file.type) {
  ...
  myDropzoneLogoUpload.on("removedfile", function (file) {
  ...
  myDropzoneLogoUpload.on("success", function (file, response) {
    var objData = JSON.parse(response);
    var currentID = document.getElementById("upload_logo_id").value;
    currentID = currentID != "" ? currentID + ";" : currentID;
    document.getElementById("upload_logo_id").value = currentID + objData.id;
  ...
  if (window.dropzoneLogo !== undefined && window.dropzoneLogo.length > 0) {
    for (var i = 0; i < window.dropzoneLogo.length; i++) {
      var file = dropzoneLogo[i];
      myDropzoneLogoUpload.options.addedfile.call(myDropzoneLogoUpload, file);
      ...
      switch (ext) {
        ...
        case "jpeg": {
          filename =
            base_url + "/channels/projects/logo/thumb_" + file.file_name;
          break;
        }
        case "doc":
        case "docx": {
      ...
      myDropzoneLogoUpload.options.thumbnail.call(
        myDropzoneLogoUpload,
        file,
        filename
      );

      myDropzoneLogoUpload.emit("complete", file);
...
Dropzone.prototype.defaultOptions.dictDefaultMessage = "Logo dự án";
