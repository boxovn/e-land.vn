...
$(function () {
  ...
  myDropzoneFileUpload.on("success", function (file, response) {
    ...
    var currentID = document.getElementById("UploadFileID").value;
    currentID = currentID != "" ? currentID + ";" : currentID;
    document.getElementById("UploadFileID").value = currentID + objData.id;
  ...
  if (window.dropzoneFile !== undefined && window.dropzoneFile.length > 0) {
    for (var i = 0; i < window.dropzoneFile.length; i++) {
      var file = dropzoneFile[i];
