...
$(function () {
  ...
  // event listeners
  var myDropzoneFileUpload = new Dropzone("#image-upload", {
    paramName: "MultipleUploadForm[files]", //paramName: "document[attachment]" Rails expects the file upload to be something like model[field_name]
  ...
  myDropzoneFileUpload.on("removedfile", function (file) {
    if (file.previewElement.id > 0) {
      ...
      $.ajax({
        url: _jsBaseUrl + "/index.php?r=house/delete-image",
        type: "post",
        data: {
          id: file.id,
        },
  ...
  myDropzoneFileUpload.on("success", function (file, response) {
    ...
    document.getElementById("UploadImageID").value = currentID + objData.id;
