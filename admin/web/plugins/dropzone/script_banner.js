...
$(function () {
  ...
  // event listeners
  var myDropzoneBannerUpload = new Dropzone("#dropzone-banner-upload", {
  ...
  myDropzoneBannerUpload.on("thumbnail", function (file, thumb) {
    file.thumbnail = thumb;
    myDropzoneBannerUpload.processQueue();
  });
  myDropzoneBannerUpload.on("sending", function (file, xhr, formData) {
    formData.append("thumbnail", file.thumbnail);
  });
  myDropzoneBannerUpload.on("addedfile", function (file, response) {
    switch (file.type) {
  ...
  myDropzoneBannerUpload.on("removedfile", function (file) {
    if (file.previewElement.id > 0) {
      var id = file.previewElement.id;
      $.ajax({
        url: _jsBaseUrl + "/index.php?r=project/remove-banner",
        type: "post",
        data: {
          id: id,
        },
  ...
  myDropzoneBannerUpload.on("success", function (file, response) {
    var objData = JSON.parse(response);
    var currentID = document.getElementById("upload_banner_id").value;
    currentID = currentID != "" ? currentID + ";" : currentID;
    document.getElementById("upload_banner_id").value = currentID + objData.id;
  ...
  if (
    window.dropzoneBanners !== undefined &&
    window.dropzoneBanners.length > 0
  ) {
    for (var i = 0; i < window.dropzoneBanners.length; i++) {
      var file = dropzoneBanners[i];
      myDropzoneBannerUpload.options.addedfile.call(
        myDropzoneBannerUpload,
        file
      );
      ...
      switch (ext) {
        ...
        case "jpeg": {
          filename =
            base_url + "/channels/projects/banner/thumb_" + file.file_name;
          break;
        }
        case "doc":
        case "docx": {
      ...
      myDropzoneBannerUpload.options.thumbnail.call(
        myDropzoneBannerUpload,
        file,
        filename
      );

      myDropzoneBannerUpload.emit("complete", file);
