    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.info.building , function(index, val) { 
                ...
                html += '<a class="box-image" title="' + val.name + '" href="' + val.href + '">';
                html += '<img class="image" alt="' + val.name + '" src="' + val.src + '"/>';
                html += '</a>';
                html += '<a title="' + val.name + '" class="title" href="' + val.href + '">' + val.name + '</a>';
                html += '<a title="' + val.address + '" class="province" href="' + val.href + '">' + val.address + '</a>';
                html += '</div>';
        });
...
function loadArticle(url) {
	$.ajax({
        url: $(location).attr("origin") + '/load-article',
        type: 'GET',
		dataType: 'json',
        ...
        success: function (data) {
          ...
          $(".loader").hide();
            $.each(data.info.building , function(index, val) { 
                ...
                html += '<a title="' + val.address + '" class="province" href="' + val.href + '">' + val.district_name +', '+  val.province_name + '</a>';
                html += '</div>';
				html += '</div>';
        });
