         ...
         //we use on so the dom element can be called again if they are nested, otherwise when we load the content once it kills the dom element and wont let you load anther modal on click without a page refresh
                ...
                $(document).on('click', '.showRegister', function(){
                    ...
                    if ($('#modalRegister').hasClass('in')) {
                        $('#modalRegister').find('.modal-content')
                                .load($(this).attr('data-url'));
                        //dynamiclly set the header for the modal
                        // document.getElementById('modalHeader').innerHTML = '<h4>' + $(this).attr('title') + '</h4>';
                    } else {
                        //if modal isn't open; open it and load content
                        $('#modalRegister').modal('show').find('.modal-content').load($(this).attr('data-url'));
                            //dynamiclly set the header for the modal
                        //  document.getElementById('modalHeader').innerHTML = '<h4>' + $(this).attr('title') + '</h4>';
                    }
            });
                // Add the modal-open class into body element
                $(document).on("show.bs.modal","#modalRegister", function (e) {
                ...
                // Add the modal-open class into body element
                $(document).on("hidden.bs.modal","#modalLogin,#modalRegisterError,#modalRegisterSuccess,#modalLoginSuccess,#modalRegister", function (e) {
